...
extends JPanel
{
    ...
    public TilePanel(SudokuPanel p, int x, int y)
    {
        ...
        {
            //Top edge where the horizontal border should be thinker
            this.setBorder(BorderFactory.createMatteBorder(1,3,1,1,Color.black));
        }
        else if(y==0 && x%3==0)
        {
            //Left edge where the vertical border should be thinker
            this.setBorder(BorderFactory.createMatteBorder(3,1,1,1,Color.black));
        }
        else if(x==0)
        {
            //Top edge all other times
            this.setBorder(BorderFactory.createMatteBorder(1,1,1,1,Color.black));
        }
        else if(y==0)
        {
            //Left edge all other times
            this.setBorder(BorderFactory.createMatteBorder(1,1,1,1,Color.black));
        }
        else if((x%3==0) && (y%3==0))
        {
            //Thicker lines for corner of a sub grid
            this.setBorder(BorderFactory.createMatteBorder(3,3,1,1,Color.black));
        }
        else if(x%3 == 0)
        {
            //Thicker lines for top edge of a sub grid
            this.setBorder(BorderFactory.createMatteBorder(3,1,1,1,Color.black));
        }
        else if(y%3 == 0)
        {
            //Thicker lines for left edge of a sub grid
            this.setBorder(BorderFactory.createMatteBorder(1,3,1,1,Color.black));
        }
        else
        {
    ...
    protected void paintComponent(Graphics g)
    {
        ...
        else
        {
            ...
            //Blue is original, red is a value added by the algorithm.
